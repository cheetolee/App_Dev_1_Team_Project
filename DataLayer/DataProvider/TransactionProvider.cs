    ...
    public class TransactionProvider
    {
        ...
        public static List<TransactionHeadListEntity> ListHead()
        {
            ...
            using (var db = new InventoryContext(DatabaseConnection.ConnectionString))
            {
                ...
                // and the user record from the Users table.
                var query = (from h in db.TransactionHeader
                             join u in db.Users on h.Username equals u.Username into PartJoin
                             from subp in PartJoin.DefaultIfEmpty()
                             select new { Head = h, User = subp });
                foreach (var record in query)
                    list.Add(new TransactionHeadListEntity(record.Head, record.User));
                list.Sort();
            }
            return list;
        }
        ...
        /// <summary>
        /// Lists every user that has transactions, and sums the total price of their transactions.
        /// </summary>
