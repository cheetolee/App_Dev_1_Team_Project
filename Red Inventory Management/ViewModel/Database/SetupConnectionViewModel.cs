...
namespace Inventory_Management.ViewModel
{
    class SetupConnectionViewModel : BindableBase
    {
        ...
        private void ConnectDatabase(object parameter)
        {
            ...
            {
                log.Info("Connected to:"+ ConnectedFile);
                SetupWindow?.Close();
            }
            else
                log.Error("Connection error:Database connection failed.");
        ...
        private void CreateDatabase(object parameter)
        {
            ...
            if (DatabaseConnection.CreateDatabase(Directory, DbName))
            {
                ...
                log.Info("Database created , New database: " + DbName);
                log.Info("Connected to:"+ ConnectedFile);
                SetupWindow?.Close();
            }
            else
                log.Error("New database error: Database creation failed.");
        }
